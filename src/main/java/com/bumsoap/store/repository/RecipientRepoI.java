...
public interface RecipientRepoI extends JpaRepository<Recipient, Long> {
  @Query(nativeQuery = true, value =
      "select r.id from recipient r where " +
  ...
  long getRecipientIdIfExist(@Param("addr_basis_id") Long addr_basis_id,
                             @Param("address_detail") Long address_detail,
                             @Param("doro_zbun") Long doro_zbun,
                             @Param("full_name") Long full_name,
                             @Param("mb_phone") Long mb_phone);
}
